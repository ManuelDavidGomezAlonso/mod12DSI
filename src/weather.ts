...
export class jsonCards {
  ...
  add(card:magicCard) {
    return new Promise((resolve, reject) => {
        try {
            fs.readFile(`${directorioUsuario}/${card.id_}.json`,(err) => {
                if (err) {
                    if (err.code === 'ENOENT') {
                        fs.writeFile(`${directorioUsuario}/${card.id_}.json`, JSON.stringify(card), () => {
                            resolve('Card added');
                        });
                    }
                }
  ...
  showCard(showIDCard: number) {
    return new Promise((resolve, reject) => {
        try {
            fs.readFile(`${directorioUsuario}/${showIDCard}.json`,(err,data) => {
                if (!err) {
